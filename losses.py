...
class ConformalLoss:
    ...
    def init_faces(self, device: torch.device) -> torch.tensor:
        ...
        for j, c in enumerate(self.target_letter):
            points_np = [self.parameters.point[i].clone().detach().cpu().numpy() for i in
                         range(len(self.parameters.point))]
...
class ISMLoss(torch.nn.Module):
    ...
    def forward(self, prompt_text, pred_rgb, grad_scale=1.0, resolution=(512, 512), step=None):
        ...
        noise = torch.randn((latents.shape[0], 4, resolution[0] // 8, resolution[1] // 8,), dtype=latents.dtype,
                            ...
                            generator=self.noise_gen) + 0.1 * torch.randn((1, 4, 1, 1), device=latents.device).repeat(
        ...
        # 随机选择当前时间步和前一个时间步。
        ind_t = torch.randint(self.min_step, self.max_step, (1,), dtype=torch.long, generator=self.noise_gen,
        ...
        with torch.no_grad():
            ...
            _, prev_latents_noisy, pred_scores_xs = self.add_noise_with_cfg(latents, noise, ind_prev_t, starting_ind,
                                                                            inverse_text_embeddings,
                                                                            self.denoise_guidance_scale,
                                                                            self.xs_delta_t, self.xs_inv_steps,
                                                                            eta=self.xs_eta)
            # Step 2: sample x_t
            _, latents_noisy, pred_scores_xt = self.add_noise_with_cfg(prev_latents_noisy, noise, ind_t, ind_prev_t,
                                                                       inverse_text_embeddings,
                                                                       self.denoise_guidance_scale, current_delta_t, 1,
        ...
        with torch.no_grad():
            ...
            unet_output = self.unet(latent_model_input.to(torch.float16), tt.to(torch.float16),
                                    encoder_hidden_states=text_embeddings.to(torch.float16)).sample
    ...
    def forward(self, x_aug):
        ...
        with torch.inference_mode():
            ...
            with torch.autocast(device_type="cuda", dtype=torch.float16):
                eps_t_uncond, eps_t = self.pipe.unet(z_in, timestep,
